...
dinamica) com no cabeca no inicio da lista.
  As listas gerenciadas podem ter um numero arbitrario de elementos.
  O no cabeca acelera o algoritmo de busta sequencial por reduzir o numero
...
/* Busca sequencial - funcao de exclusao (retorna no endereco *ant o indice do
   ...
   O no cabeca pode ser o anterior.
*/
PONT buscaSeqExc(TIPOCHAVE ch, LISTA *l, PONT *ant){
  *ant = l->cabeca;
  PONT pos = l->cabeca->prox;
  l->cabeca.chave = ch; // utiliza o cabeca como sentinela
  while (pos->chave<ch)){
    *ant = pos;
    pos = pos->prox;
  }
