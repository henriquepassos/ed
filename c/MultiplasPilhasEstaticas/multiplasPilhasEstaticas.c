...
typedef struct {
  int base[NP+1]; // pilhas [0..NP-1] + pilha[NP] auxiliar (a "pilha[NP]" facilitara a verificacao se a pilha NP-1 esta completa)
  int topo[NP+1];
  TIPOCHAVE A[MAX];
} PILHAMULTIPLA;

// Inicializacao da pilha multipla
...
// Desloca pilha k uma posicao para a direita, se possivel
bool paraDireita(PILHAMULTIPLA *p, int k) {
  int i;
  if( (k < 1) || (k > NP-1) ) return (false); // indice invalido
...
// Desloca pilha k uma posicao para a esquerda, se possivel
...
bool pushK(TIPOCHAVE ch, PILHAMULTIPLA *p, int k) {
  ...
  if (pilhaKcheia(p,k)){
    ...
    if((pilhaKcheia(p, k)) && (k > 0)) // se a pilha continua cheia
        // desloca p/esquerda todas as pilhas de [1..k] (mas nao a pilha 0)
