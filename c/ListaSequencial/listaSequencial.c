...
/* Insercao "direta" na iesima posicao (posicao i-1 do arranjo A).
...
/* Busca sequencial em lista ordenada ou nao SEM SENTINELA */
...
/* Busca sequencial em lista COM SENTINELA (vetor criado com MAX+1 posicoes) */
int buscaSent(TIPOCHAVE ch, LISTA *l) {
  int i = 0;
  l->A[l->nroElem].chave = ch; // sentinela
  while(l->A[i].chave != ch) i++;
  if (i > l->nroElem -1) return ERRO; // nao achou
  else return(i);
} /* buscaSent */
...
/* Insercao em lista ordenada COM SENTINELA sem duplicacao */
...
/* Exclusao do elemento cuja chave seja igual a ch */
bool excluirElemLista(TIPOCHAVE ch, LISTA *l)
{
  int pos, j;
  pos = buscaSeq(ch, l);
  if(pos == ERRO) return(false); // nao existe
